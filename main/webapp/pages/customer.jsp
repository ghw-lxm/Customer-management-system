...
<!-- WARNING: Respond.js doesn't work if you view the paging via file:// -->
<!--[if lt IE 9]>
...
<body>
	...
	<div id="wrapper">
		...
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="index.html">SpringCRM v1.0</a>
		</div>
		<!-- /.navbar-header -->
		...
		<ul class="nav navbar-top-links navbar-right">
			...
			<li class="dropdown"><a class="dropdown-toggle"
				...
				<ul class="dropdown-menu dropdown-user">
					...
					<li class="divider"></li>
					<li><a href="userLogout"><i class="fa fa-sign-out fa-fw"></i>
							退出登录</a></li>
				</ul> <!-- /.dropdown-user --></li>
			<!-- /.dropdown -->
		</ul>
		...
		<div id="page-wrapper">
			...
			<div class="panel panel-default">
				<div class="panel-body">
					<form id="form" class="form-inline" action="${pageContext.request.contextPath }/customerList" method="get">
						<div class="form-group">
							<label for="customerName">客户名称</label> 
							<input type="text" class="form-control" id="customerName" value="${customerQuery.custName }" name="custName">
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="customerFrom" placeholder="客户来源" name="custSource">
								...
								<c:forEach items="${fromType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == customerQuery.custSource}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custIndustry"  name="custIndustry">
								...
								<c:forEach items="${industryType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == customerQuery.custIndustry}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custLevel" name="custLevel">
								...
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == customerQuery.custLevel}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							<label for="customerName">每页条数</label>
							<input style="width:50px" type="text" class="form-control" id="size" value="${customerQuery.size}" name="size">
						</div>
						<div class="form-group">
							<label for="customerName">页码</label>
							<input style="width:50px" type="text" class="form-control" id="page" value="${customerQuery.page}" name="page">
						</div>
